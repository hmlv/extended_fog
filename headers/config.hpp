...
class segment_config{
    public:
        ...
        //when calculate the new vertex attribute, the vertex need all its neighbors, we don't need the update buffer
        //added by Huiming Lv
        //date: 2017/03/06
        segment_config(const char* buf_head, u64_t per_cpu_info_size)
        {
            //still assume the vertex id starts from 0
            u32_t num_vertices = gen_config.max_vert_id + 1;

            buf0_holder = buf1_holder = -1;
            //in this scenario, fog will divide the whole (write) buffer into 3 pieces (in theory):
            //	following figure explains why there are 3 pieces
            //	---------------------------------		---
            //  |	sched_update (for each CPU)	|
            //	---------------------------------		sched_update_buf_len
            //  |	sched_update (for each CPU)	|
            ...
            for( u32_t i=0; i<gen_config.num_processors; i++ ){
                per_cpu_info_list[i] = new per_cpu_data<VA>;
                per_cpu_info_list[i]->buf_head = (char*)((u64_t)buf_head+i*per_cpu_info_size);
                per_cpu_info_list[i]->buf_size = per_cpu_info_size;
            }

            char * attr_buf_head = (char*)((u64_t)buf_head + per_cpu_info_size * gen_config.num_processors);
            u64_t graph_attr_size = (u64_t)(ROUND_UP( num_vertices, gen_config.num_processors ) * sizeof(VA));
            u64_t remain_size     = gen_config.memory_size - per_cpu_info_size * gen_config.num_processors;

            if( graph_attr_size < remain_size ){	//small graph, only give one attr_buffer
                num_attr_buf = 1;
                //gen_config.scope_of_attr = 0;
                PRINT_DEBUG("This is a small graph!\n");
                attr_buf_len = graph_attr_size;
                //attr_buf0 = (char*)((u64_t)buf_head + (gen_config.memory_size - graph_attr_size ) );
                attr_buf0 = attr_buf_head;
                ...
                // however, it is not to find an exact division for the whole attribute data! since
                // this can possibly make the attribute buffer very small.
                u64_t segment_size = ROUND_DOWN(remain_size / 2, (sizeof(VA)*gen_config.num_processors));
                u64_t old_remain, new_remain;
...
struct task_config{
    ...
    task_config(bool m_is_remap, const struct task_config * main_task_config, const int id, const std::string & desc_data_name){
        this->is_remap = m_is_remap;
        init(desc_data_name);
        this->vec_prefix_task_id = main_task_config->vec_prefix_task_id;
        this->vec_prefix_task_id.push_back(id);
    }
    task_config(bool m_is_remap, int id, const std::string & desc_data_name){
        this->is_remap = m_is_remap;
        init(desc_data_name);
        this->vec_prefix_task_id.push_back(id);
    }
    private:
        inline void init(const std::string & desc_data_name){
            ...
            this->vert_file_name    = desc_data_name.substr(0, desc_data_name.find_last_of(".")) + ".index";
            this->edge_file_name    = desc_data_name.substr(0, desc_data_name.find_last_of(".")) + ".edge";
            this->attr_file_name    = desc_data_name.substr(0, desc_data_name.find_last_of(".")) + ".attr";
            if(gen_config.with_in_edge){
                this->in_vert_file_name = desc_data_name.substr(0, desc_data_name.find_last_of(".")) + ".in-index";
                this->in_edge_file_name = desc_data_name.substr(0, desc_data_name.find_last_of(".")) + ".in-edge";
            }
            this->with_in_edge      = gen_config.with_in_edge;
        }
};
